...
token="1279607371:AAHF7SARLIS0UzHJYK5CwgrlIbVLTzaomrA"
bot=telebot.TeleBot(token)
admin=[635256922,1066849301, 1283541512]
group="@Myrkgr"
channel="@testforce1"
...
def start_markup():
    ...
    share=InlineKeyboardButton('♻️ Share Bot',switch_inline_query=' Pink Promotion provides you the best cross promotion services! Add your channel and Participate in Promotions Join Now @PinkPromotion')
...
def my_channel_markup():
    ...
    remove=InlineKeyboardButton('⛔️ Remove Channel',callback_data='remove_channel')
...
def handler_query(call):
    ...
    if call.data=="send_morden_promo":
        try:
            ...
            channl=""
            unsucess=""
            qe=bot.send_message("PinkPromotionDaily",dest,reply_markup=promo_button_markup(),parse_mode='HTML',disable_web_page_preview=True)
            f=open("save.txt","w")
    ...
    if call.data=="send_button_promo":
        try:
            ...
            unsucess=""
            us='*___________________________*'
            qe=bot.send_photo("@PinkPromotionDaily",open('a.jpg','rb'),caption=b.set_caption,reply_markup=button_markup(),parse_mode='HTML')
            f=open("save.txt","w")
            f.write(str(qe.message_id))
            f.close()
            for i in a:
    ...
    if call.data=='send_classic_promo':
        try:
            ...
            try:
                ...
                bot.reply_to(message, other)
            try:
                bot.send_message(group,dg+unsucess)
            except Exception as error:
                bot.reply_to(message, error)
        except ApiException as api:
            bot.reply_to(message, api)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                a=bot.get_chat_member(message.forward_from_chat.id,1279607371)
                if a.status =='administrator':
                    member=bot.get_chat_members_count(str(message.forward_from_chat.id))
                    if member>=1:
                        channel_data(message)
                    else:
                        bot.send_message(message.chat.id,"Minimum 1 needed for Register")
